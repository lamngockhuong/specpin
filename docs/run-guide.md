...
Options has a **Matching corpus (local, opt-in)** card to help tune the scorer against real drift. It is **off by default**; when enabled, Specpin records matching-drift data on your device only (`storage.local`, capped) - never uploaded. Two things feed it: re-pinning an orphaned/scored spec records the correction, and specs that go orphaned or scored at match time contribute a sanitized snapshot of nearby candidate elements. Only fingerprints are stored (no page HTML), with obvious PII in captured text (emails, long digit runs) masked at write time. The card shows the entry count and lets you **Export corpus (JSON)** (a local download) or **Clear corpus** (with a confirm). On a scored match's pinned tooltip you can also click **Correct** to affirm the match is right, feeding the same corpus.
