...
Markers only appear on a page that a project actually serves (a connected sidecar or a local project matching the origin) — coverage points you at gaps you can *author into*, so on a site with no project it stays silent even while the mode is globally on.
