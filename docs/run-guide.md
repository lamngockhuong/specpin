...
Each rendered spec shows how confidently it matched its element. An exact match (resolved by a `data-spec-id`/test-id, `aria-label`, or a stable `id`) is silent - the good case stays quiet. A lower-confidence match (resolved only by a CSS selector) gets a **Selector match** badge with a "why matched" hint. When exact and selector matching both fail, a **hybrid scorer** weighs the fingerprint's signals (text, nearby labels, attributes, structure, position) against the live DOM and, if one element is a confident, unambiguous winner, renders a **Scored match** badge showing its confidence and dominant signal; a mid-confidence scored match reads as needs-review. Badges appear in the tooltip/sidebar/modal and on the side-panel card. The popup and side panel also show a one-line **page health summary** (`N specs · X exact · Y scored · Z fuzzy · W orphaned`), and the side panel lists any **orphaned** specs - specs pinned to this page (by their `pageUrl` scope) whose element is no longer on the page, so you know a doc exists but its anchor is gone. A spec authored before its element exists (e.g. via the extension's specshot page, opened with the popup/side panel's **Open spec sheet** action in the header's **⋯ More actions** menu - see `docs/spec-sheet-authoring.md`) has no fingerprint yet; it never renders on the page, and instead appears read-only in an **Unpinned** section of the popup/side panel, distinct from orphaned (which has a fingerprint that failed to match).
...
Press `Alt+Shift+U` to toggle **coverage mode**: dashed ghost "+" markers appear on every *undocumented interactive element* on the page (buttons, links with href, inputs/select/textarea, ARIA widget roles like button/link/checkbox/tab/menuitem/combobox/slider, elements with `onclick`, `tabindex >= 0`, or `contenteditable`). Hidden elements, display:none, zero-size, disabled, or `aria-disabled` elements are never marked - nor are visually-hidden controls (sr-only clipping, `opacity:0`, or off-screen positioning such as `left:-9999px`).

Coverage only appears on a page that a project actually serves (a connected sidecar or a local project matching the origin) - it points you at gaps you can *author into*, so on a site with no project both the on-page markers and the popup/side-panel summary stay silent even while the mode is globally on. Turning Specpin off (the popup toggle, `Alt+Shift+S`, or the right-click **Turn off Specpin**) hides coverage as well, markers and summary alike, in lock-step with every other Specpin surface; turn it back on and they return.
...
- **Ignore** (appears only when the element has a stable anchor): Dismisses the marker as a personal choice (stored in `storage.sync` per origin), so the gap stays hidden after reload and on other machines. Dismissing a gap updates the coverage summary and the **Capture all gaps (K)** count immediately in any open popup or side panel.
...
Specs render as a **tooltip** (hover peek), a **sidebar** (persistent list), or a **modal** (a draggable, non-blocking panel listing every spec on the page). A pinned tooltip (click its badge) is height-capped with an internal scrollbar for long specs and can be resized by dragging its bottom-right corner; each pin opens at the default size. When a badge sits on top of something you need to see, drag the badge itself to move it aside (a plain click still pins); the moved badge stays put and stops re-anchoring on scroll, and the position is temporary (not saved, resets on reload). The modal opens centered but you can drag it by its header to anywhere on screen, and the page behind stays interactive (no dimming overlay) so you can keep it open while you work. Switch with the popup's mode dropdown or cycle with `Alt+Shift+M`. Per-spec `preferredDisplayMode` and the manifest `defaultDisplayMode` still apply when no mode is forced.
...
**From files (no JSON assembly).** Click the file picker, select `manifest.json` plus one or more `*.spec.json` files from your `.specs/` directory, then **Load from files**. You can also include the optional `.specs/` config files in the same pick: `guides.json`, `views.json`, and `required.json`. The extension assembles and validates everything in-page, so importing a whole `.specs/` folder brings its guides and views along, not just the specs. Each config behaves like it does under `specpin serve`:

- `guides.json` - the imported guides render as team guides for the batch's pages (the schema caps a project at 50 guides; a file over the cap is rejected with a clear error, like any other schema-invalid config).
- `views.json` - its `hidden` facets hide the matching specs on the batch's pages, exactly like a sidecar's team-default views.
- `required.json` - accepted and stored, but currently inert in the extension (it is a `specpin validate` coverage checklist read only by the CLI; there is no in-extension effect yet).

You can also pick an exported `<project>.specs.zip` directly, or a folder zipped with any normal tool (the same picker accepts a zip, alone or alongside loose files): the extension unzips it and validates its contents through the same path. Both uncompressed (STORE) and compressed (DEFLATE) zips are supported; a truly corrupt archive reports a clear error.
...
3. **Export** the project (popup/panel **Export**, or per-batch **Export** in Options) to a `<project>.specs.zip` containing `manifest.json` + one `*.spec.json` per group, plus any of the `.specs/` config files that hold content: `guides.json` (team guides), `views.json` (hidden facets), and `required.json` (local projects only; the sidecar does not export it). An empty config file is omitted. Unzip it into a repo's `.specs/`, or re-import it straight through the multi-file picker (drop the `.specs.zip` in, or its unzipped files) - the round-trip preserves group names, spec content, guides, and views, and `specpin serve` reads the result.
...
**Enable it.** Auto-capture is **per project**, not device-wide - so a stabilized project need not keep recording. Turn it on for a specific project either from its row on the extension Options page (each project row has a **Record** switch beside its enable/disable switch) or from the graph panel while reviewing that project (the recording banner's **Turn on** action). A project records **only on the pages it serves** - navigation on any other site is ignored and never stored. The Options page's **Support** pane has an **FAQ** entry explaining the auto-capture privacy contract. The graph panel (opened via **Open graph view** in the popup or side panel) shows a per-project recording banner with a **Turn on**/**Turn off** control and a **Clear all captured** action for the currently-selected project, so you can manage recording right where you review it.
...
**Ignore noisy routes.** Menu, sidebar, and utility navigation (e.g. `/settings`, `/help`) can flood the review with ghost edges you never want in the flow. A ghost edge's **Approve / Discard** panel also offers **Ignore route**: it adds the destination screen's URL glob to that project's per-project ignore-list, drops every draft edge already on that route, and stops the recorder from re-capturing it. Manage the list from the project's row on the Options page - an **Ignored routes** editor appears under a recording project, where you add a glob (same `/settings/**` syntax as page-scope globs) or remove one to record that route again. A transition is dropped when **either** endpoint matches, so an ignored screen leaves the flow entirely.
...
**Turn it on.** Click **Edit mode** in the graph view's control bar. A dedicated edit bar opens below the controls with the toolbar (**Add node**, **Add edge**, **Delete selected**, **Undo**, **Save**) plus a how-to hint and a live status line. Each button enables only when its precondition is met - **Add edge** waits until two nodes are selected, **Delete selected** until exactly one node or edge is, and **Undo**/**Save** until the draft has an unsaved change - so a greyed-out button tells you what the action still needs. Clicking a node or edge now selects it for editing instead of navigating or click-to-highlighting. Auto-captured **ghost** (pending) transitions stay on the Screens diagram in edit mode too - they don't vanish when you toggle Edit on - and clicking one still opens its **Approve / Discard** panel rather than the edit form, so a recording stays reviewable while you edit (approve it first to turn it into a normal editable edge).
...
**Edit a node or edge.** Click an existing one to open the same side form, pre-filled. Every valid field change applies to the in-memory draft immediately (the graph re-renders shortly after); **Save** is still what persists the draft to `.specs/`. An imported transition (`"source": "imported"`, flows dataset only) renders read-only here - reassign that through code-import instead. An auto-captured navigation edge (Screens dataset) is directly editable: editing any of its fields reclassifies it to `"source": "manual"` on Save, so re-running auto-capture no longer touches it afterwards.
...
> **Provenance, mostly unchanged:** every write here goes through the same read-merge-validate-write path as auto-capture's Approve and code-import's writer - manual edits are stamped `"source": "manual"` and never overwrite an imported entry, and an imported transition (flows dataset) stays read-only in this editor. The one change: editing or deleting an auto-captured navigation edge in the Screens dataset (or a screen it points at) reclassifies it to `"source": "manual"` on Save, automatically and with no separate step - from then on it's a manual entry that auto-capture no longer manages, reviewable through the normal `.specs/` git diff like any other manual edit. The editor adds no new schema and no new write surface; it is a UI on top of the same `.specs/` files.
