...
- Spec deletion: a writable spec (sidecar or local) is deletable from the pinned tooltip (`pin-delete`) or a side-panel card (`spec-delete`) behind a destructive confirm; `DELETE_SPEC` is unprivileged + origin-routed (same trust model as `UPDATE_SPEC`), routing to `registry.deleteSpec` (sidecar) or `deleteLocalSpec` -> `removeLocalSpecById` (local, `batchServesOrigin`-guarded); side-panel Delete delegates to the page via `DELETE_SPEC_HERE`.
