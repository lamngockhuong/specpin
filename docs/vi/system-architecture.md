...
.specs/ (consumer repo)
   |  read/write JSON
   v
specpin serve  ── Go sidecar ──  HTTP + SSE (token-authenticated; mặc định 127.0.0.1, tùy chọn remote qua HTTPS reverse proxy)
   ^
...
- **Personal override** trong `chrome.storage.sync` (per profile trình duyệt, qua máy). Một personal force-show của `spec:<id>` là một hard rescue per-spec (được ưu tiên hơn hide theo tag/file). Tag/file/url force-show chỉ bỏ hide key của chính nó. Cổng `url:` page ghi đè mọi thứ.
...
- Mặc định bind `127.0.0.1`; port được auto-pick trừ khi truyền `--port`. `--host <addr>` có thể bind một địa chỉ non-loopback để dùng từ xa, nhưng như vậy sẽ phơi trực tiếp cổng plaintext chỉ-token ra ngoài (reverse proxy không nằm trong luồng) và in ra một cảnh báo, nên hãy đặt firewall cho nó. Client từ xa phải kết nối tới sidecar qua **HTTPS thông qua một reverse proxy** (extension chặn plaintext từ xa vì bị coi là mixed content); bản thân Go binary không terminate TLS. Xem `docs/run-guide.md` mục "Serve on a remote machine".
- Mọi request đều yêu cầu `Authorization: Bearer <token>` (được in ra khi `serve`). Token là ngẫu nhiên cho mỗi lần chạy trừ khi ghim bằng `--token`/`SPECPIN_TOKEN`. **Token là ranh giới authorization duy nhất cho các network client không phải trình duyệt**: CORS chỉ ràng buộc origin của trình duyệt, nên một client không có `Origin` (ví dụ `curl`) chỉ được kiểm soát bằng token.
- CORS chỉ chấp nhận extension origin (`chrome-extension://`, `moz-extension://`, `safari-web-extension://`); các web origin bị từ chối. Preflight cho phép `Authorization`, `Content-Type`, và `If-Match`, đồng thời expose `ETag` để một client cross-origin (từ xa) có thể đọc được.
