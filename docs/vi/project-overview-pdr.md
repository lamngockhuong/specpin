...
3. **Render dạng pluggable**: tooltip (xem nhanh), sidebar (đọc tập trung), overlay/modal (chỉnh sửa toàn màn hình), inline badge (đánh dấu trực quan). MVP ship kèm tooltip + sidebar.
...
- **Hai renderer**: tooltip (hover để xem nhanh) và sidebar (panel đọc/ghi cố định).
- **Manual capture mode**: click một element, điền form (title, description, rule), lưu vào `.specs/`.
- **Fingerprinting exact-match**: thử lần lượt test-id anchor, aria, id không phải sinh tự động, cssSelector duy nhất, xpath. Đánh dấu `needsReview` khi mơ hồ. Thuộc tính `data-spec-id` đảm bảo match exact.
...
## Non-Goals (hoãn sang 1.1+)

- Capture có AI hỗ trợ (`specpin generate`) - không phụ thuộc LLM trong MVP.
- Source adapter dùng FileSystem Access API - MVP chỉ dùng sidecar.
- Hybrid weighted fingerprint scoring - các anchor exact đã đủ cho MVP.
- Đóng gói Safari - MVP chỉ có Chrome + Firefox.
- Các renderer overlay, modal, inline-badge - tooltip + sidebar đã đủ cho MVP.
- Source import CSV/JSON thủ công - chỉ ghi qua extension.
...
- Sidecar hosted/cloud (MVP chỉ localhost).
...
**Phase 1.1 (đã lên kế hoạch):**
- Source FileSystem + Manual để import các spec sẵn có.
- Hybrid weighted fingerprint scoring để bền bỉ hơn khi refactor.
...
- **Hiệu năng**: bundle content script của extension < 500 KB chưa nén (MVP: ~450 KB với ajv). Fingerprint match < 50ms mỗi element (MVP: < 10ms với các anchor exact). Độ trễ render < 100ms sau khi match.
...
- **Tương thích**: Node >= 20, pnpm 10, Go 1.26, Chrome 120+ (MV3), Firefox 115+ (MV2 compat). Fingerprinting thuần DOM (không coupling với framework).
...
- **License**: Apache-2.0 (quyết định khi hoàn thành Phase 1).
...
| Fingerprint dễ vỡ khi refactor | High | Ưu tiên các anchor exact (test-id, aria, data-spec-id); hybrid scorer hoãn lại nhưng interface ổn định | Deferred (1.1) |
| Bundle content-script của extension phình to | Medium | Validator ajv (~100 KB) nằm trong content script; cân nhắc chuyển validation sang SW ở 1.1 | Accepted (MVP) |
| Xung đột port khi dev nhiều project | Low | Tự chọn port trống trừ khi có override --port | Implemented |
| Rò rỉ spec cross-origin sang subdomain nhái | High | Chỉ match subdomain dạng host-exact hoặc theo ranh giới label; có regression test | Fixed (Phase 1) |

## Câu hỏi chưa giải quyết

1. **Thiết kế hybrid fingerprint scorer**: dùng signal nào, weight ra sao, ngưỡng confidence nào kích hoạt `needsReview`? Hoãn sang 1.1; đã có interface placeholder (shape `MatchResult` ổn định).
2. **UX cấp quyền FileSystem Access API**: prompt người dùng cấp quyền truy cập thư mục `.specs/` thế nào mà không phá vỡ capture flow? Hoãn sang 1.1.
3. **Timeline đóng gói Safari**: mức độ parity MV3 chưa rõ tính tới 2026-06. Chờ Apple làm rõ.
4. **Capture có AI hỗ trợ (`specpin generate`)**: model nào, prompt dạng gì, local hay cloud, quản lý key ra sao? Hoãn sang 1.1; chưa chốt quyết định nào.
...
- Journal Phase 1: `docs/journals/260625-specpin-phase1-mvp.md`
