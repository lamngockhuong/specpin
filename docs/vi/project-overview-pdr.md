...
- Hybrid weighted fingerprint scoring - hiện anchor exact + selector duy nhất đã đủ; interface `MatchResult` giữ ổn định cho một scorer về sau.
...
| Fingerprint dễ vỡ khi refactor | High | Ưu tiên các anchor exact (test-id, aria, data-spec-id); hybrid scorer dự kiến làm nhưng interface ổn định | Planned |
...
1. **Thiết kế hybrid fingerprint scorer**: dùng signal nào, weight ra sao, ngưỡng confidence nào kích hoạt `needsReview`? Dự kiến; đã có interface placeholder (shape `MatchResult` ổn định).
