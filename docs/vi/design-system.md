...
- **Icon buttons**: Bộ icon SVG inline dùng chung từ `src/shared/icons.ts` (viewBox 12x12, stroke bằng currentColor). Các icon bao gồm: close (X), plus (+), check, play, pencil, trash. Dùng cho nút đóng ở phần đầu modal (X), các action Start/Edit/Delete trên guide-row (icon play/pencil/trash), nút Done/Cancel của HUD, và nút Delete per-entry của matching-corpus trong Options (icon trash). Luôn đi kèm `aria-label` và `title` để truy cập được và hiện tooltip khi hover.
