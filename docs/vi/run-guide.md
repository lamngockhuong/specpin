...
Mỗi spec được render cho thấy nó đã match element với độ tin cậy ra sao. Match chính xác (được giải bằng `data-spec-id`/test-id, `aria-label`, hoặc một `id` ổn định) thì im lặng - trường hợp tốt giữ yên tĩnh. Match độ tin cậy thấp hơn (chỉ giải được bằng CSS selector) nhận một badge **Selector match** kèm gợi ý "vì sao match". Khi cả match chính xác lẫn selector đều thất bại, một **hybrid scorer** cân các signal của fingerprint (text, nhãn lân cận, thuộc tính, cấu trúc, vị trí) với DOM sống và, nếu một element là ứng viên thắng rõ ràng và không mơ hồ, render một badge **Scored match** hiển thị độ tin cậy và signal nổi trội của nó; một scored match độ tin cậy trung bình đọc như cần-xem-lại. Badge xuất hiện trong tooltip/sidebar/modal và trên spec card của side panel. Popup và side panel cũng hiển thị một dòng **tóm tắt sức khỏe trang** (`N specs · X exact · Y scored · Z fuzzy · W orphaned`), và side panel liệt kê mọi spec **mồ côi** - spec được ghim vào trang này (theo phạm vi `pageUrl` của nó) nhưng element không còn trên trang, để bạn biết tài liệu vẫn tồn tại nhưng neo của nó đã mất.
...
Options có một thẻ **Matching corpus (local, opt-in)** để giúp tinh chỉnh scorer với drift thực tế. Mặc định **tắt**; khi bật, Specpin ghi dữ liệu drift khớp chỉ trên thiết bị của bạn (`storage.local`, có giới hạn) - không bao giờ tải lên. Hai thứ nạp vào nó: re-pin một spec mồ côi/scored ghi lại phần sửa, và các spec trở nên mồ côi hoặc scored lúc match đóng góp một snapshot đã làm sạch của các element candidate lân cận. Chỉ fingerprint được lưu (không có HTML trang), với PII rõ ràng trong text đã bắt (email, chuỗi số dài) được che lúc ghi. Thẻ hiển thị số mục và cho phép **Export corpus (JSON)** (tải về cục bộ) hoặc **Clear corpus** (kèm xác nhận). Trên tooltip đã ghim của một scored match bạn cũng có thể bấm **Correct** để xác nhận match là đúng, nạp vào cùng corpus đó.
