...
**Mở nó.** Click **Open graph view** trong menu **⋯ More actions** ở header của popup hoặc side panel; nó mở graph trong một tab mới. Khi một project đã kết nối phục vụ nhiều hơn một dataset, một bộ chọn project/dataset xuất hiện phía trên canvas (bộ chọn dataset chỉ hiện khi một project có **cả** flows lẫn screens được cấu hình). Lựa chọn project/dataset của bạn được ghi nhớ qua mỗi lần tải lại (và khi mở lại tab graph), nên F5 sẽ đưa bạn về đúng khung nhìn cũ thay vì reset về project đầu tiên; nếu project đó không còn (ví dụ đã ngắt kết nối) thì tự quay về project đầu tiên.
