...
- **Bỏ qua** (xuất hiện khi element có neo ổn định): Loại bỏ dấu như một lựa chọn cá nhân (lưu trong `storage.sync` theo origin), nên gap vẫn bị ẩn sau lần reload và trên các máy khác. Khi bạn bỏ qua một gap, tóm tắt coverage và số đếm **Capture all gaps (K)** cập nhật ngay lập tức trong popup hoặc side panel đang mở.
...
Node/edge không mang `specId` (một state thuần domain như `"won"`, hoặc một điều hướng không có phần tử kích hoạt riêng lẻ) vẫn render bình thường nhưng không có gì để click qua.
...
`screens.json` (xem [Graph views](#19-graph-views)) không chỉ lớn lên nhờ soạn tay hay code-import: extension còn có thể **quan sát chính việc điều hướng của bạn khi duyệt web** và đề xuất các screen transition mới để xem xét, ngay trong graph panel. Tính năng này **tự chọn tham gia (opt-in) và mặc định tắt** - hãy đọc cam kết riêng tư bên dưới trước khi bật.

**Bật nó.** Tự động ghi bật **theo từng project**, không phải cho cả thiết bị - nên một project đã ổn định thì không cần tiếp tục ghi. Hãy bật cho một project cụ thể ngay tại dòng của nó trên trang Options (mỗi dòng project có công tắc **Ghi** bên cạnh công tắc bật/tắt), hoặc từ graph panel khi đang xem xét project đó (nút **Bật** trên banner ghi hình). Một project chỉ ghi **trên những trang mà nó phục vụ** - điều hướng trên các trang khác bị bỏ qua và không bao giờ được lưu. Pane **Hỗ trợ** của trang Options có một mục **FAQ** giải thích cam kết riêng tư của tự động ghi. Graph panel (mở qua **Open graph view** ở popup hoặc side panel) hiển thị banner ghi hình theo từng project với nút **Bật**/**Tắt** và một hành động **Xóa tất cả đã ghi** cho project đang được chọn, nên bạn có thể quản lý việc ghi ngay tại nơi mình xem xét nó.

**Những gì được và không được ghi lại.** Được ghi lại: dạng đường dẫn màn hình đã tổng quát hóa cho mỗi trang bạn ghé qua (ví dụ `/orders/**`, không bao giờ là `/orders/1938` cụ thể) và lượt điều hướng giữa hai màn hình như vậy. Không bao giờ ghi lại: query string, hash, hay nội dung trang - chỉ *hình dạng* URL. Các đoạn path trông giống id (số, UUID, mã ngắn xen chữ-số, v.v.) sẽ được tổng quát hóa thành `**` trước khi lưu bất cứ thứ gì, nhưng đây là suy đoán chứ không phải một cam kết tuyệt đối - hãy xem lại từng transition trước khi Duyệt. **Không có gì được ghi vào `.specs/` tại thời điểm ghi hình.** Mỗi transition quan sát được trước tiên rơi vào một bộ đệm nháp cục bộ theo từng project (`storage.local`, không bao giờ tải lên, có giới hạn theo project), và vẫn chỉ là đề xuất cho đến khi bạn hành động rõ ràng với nó.

**Xem xét và duyệt.** Duyệt trang web với chế độ ghi đang bật, rồi mở dataset **Screens** của graph view: các screen và transition mới quan sát được render dưới dạng node/edge "ghost" nét đứt, hơi trong suốt, xen giữa các node/edge đã được lưu - đủ khác biệt về mặt hình ảnh để "đang chờ và chưa xác nhận" không bao giờ bị nhầm là đã lưu. Click vào một ghost edge để mở bảng **Duyệt / Bỏ qua** nội tuyến. **Duyệt** xác thực và gộp nó vào `screens.json` với `"source": "auto-captured"` (dùng đúng code gộp bảo toàn provenance mà code-import và soạn tay dùng - không bao giờ ghi đè lên một entry manual/imported đã có cùng id) rồi entry rời khỏi bộ đệm nháp; **Bỏ qua** chỉ xóa nó khỏi bộ đệm, không ghi gì vào `.specs/` ở cả hai trường hợp. Graph panel cũng báo cho bạn biết khi đang ghi nhưng bộ đệm còn trống (hãy duyệt trang để ghi được gì đó), và khi bộ đệm của một project đã chạm giới hạn (hãy duyệt hoặc bỏ qua bớt trước khi có thể ghi thêm).

**Bỏ qua các route gây nhiễu.** Điều hướng qua menu, sidebar và các trang tiện ích (ví dụ `/settings`, `/help`) có thể làm phần xem xét ngập ghost edge mà bạn không hề muốn đưa vào flow. Panel **Duyệt / Bỏ qua** của một ghost edge còn có thêm **Bỏ qua route**: nó thêm URL glob của màn hình đích vào danh sách bỏ qua theo từng project, xóa mọi edge nháp đang nằm trên route đó, và ngừng để recorder ghi lại nó. Quản lý danh sách này ngay tại dòng của project trên trang Options - một trình sửa **Route bỏ qua** hiện ra dưới một project đang ghi, nơi bạn thêm một glob (cùng cú pháp `/settings/**` như glob phạm vi trang) hoặc xóa một glob để ghi lại route đó. Một transition bị loại khi **một trong hai** đầu khớp, nên một màn hình bị bỏ qua sẽ rời khỏi flow hoàn toàn.
...
**Sửa một node hoặc edge.** Click vào một cái đã có để mở đúng form đó, đã điền sẵn dữ liệu. Mọi thay đổi hợp lệ áp dụng vào bản nháp trong bộ nhớ ngay lập tức (graph sẽ vẽ lại sau một khoảng ngắn); **Save** vẫn là bước lưu bản nháp xuống `.specs/`. Một transition imported (`"source": "imported"`, chỉ có ở dataset flows) hiện ở đây dạng chỉ đọc - hãy đổi nó qua code-import thay vào đó. Một edge điều hướng auto-captured (dataset Screens) thì sửa được trực tiếp: sửa bất kỳ trường nào của nó sẽ chuyển nó sang `"source": "manual"` khi Save, nên sau đó chạy lại auto-capture sẽ không còn động tới nó nữa.
...
> **Provenance, gần như không đổi:** mọi lượt ghi ở đây đi qua đúng luồng đọc-gộp-xác thực-ghi mà Duyệt của auto-capture và bộ ghi của code-import dùng - chỉnh sửa thủ công được đóng dấu `"source": "manual"` và không bao giờ ghi đè lên một entry imported, và một transition imported (dataset flows) vẫn chỉ đọc trong trình biên tập này. Thay đổi duy nhất: sửa hoặc xóa một edge điều hướng auto-captured ở dataset Screens (hay một screen mà nó trỏ tới) sẽ chuyển nó sang `"source": "manual"` khi Save, tự động và không cần thao tác riêng - từ đó nó là một entry manual mà auto-capture không còn quản lý, xem lại được qua git diff `.specs/` thông thường như mọi chỉnh sửa manual khác. Trình biên tập không thêm schema mới, không thêm bề mặt ghi mới; nó chỉ là một lớp giao diện trên cùng những file `.specs/` đó.
