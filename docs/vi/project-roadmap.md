...
- Một chế độ ghi hình opt-in, **mặc định TẮT** (Options -> **Tự động ghi**) quan sát điều hướng SPA + full-page và suy ra một screen transition đã ẩn danh cho mỗi lượt điều hướng: một `urlGlob` tổng quát hóa cho mỗi screen (bỏ query và hash; các đoạn path trông giống id được tổng quát hóa thành `**` - dùng đúng cách tổng quát hóa mà adapter import `react-router` dùng) và edge giữa chúng. Transition ghi được đưa vào một bộ đệm nháp cục bộ, theo từng project, có giới hạn trong `storage.local` (`background/capture-buffer.ts`) - không bao giờ ghi vào `.specs/` và không bao giờ tải lên.
...
- UX riêng tư/đồng thuận: chỉ báo **Đang ghi điều hướng** không thể bỏ lỡ (chấm nhấp nháy) trên cả thẻ Options lẫn banner của graph panel, mỗi nơi đều có một điều khiển **Tắt** ngay trong tầm với; một tuyên bố riêng tư trong sản phẩm nêu rõ những gì được/không được ghi lại và rằng không có gì chạm tới `.specs/` trước khi Duyệt rõ ràng; một hành động **Xóa tất cả đã ghi** (theo từng project) cho bộ đệm nháp; và thông báo trạng thái bộ đệm rỗng/đầy trong banner của graph panel.
- Message có đặc quyền mới `CLEAR_CAPTURE_BUFFER`/`APPROVE_CAPTURED_TRANSITION`/`DISCARD_CAPTURED_TRANSITION` (chỉ từ trang extension) cùng với `RECORD_CAPTURED_TRANSITION`/`GET_CAPTURE_BUFFER` không đặc quyền; background phân giải project sở hữu một lượt ghi từ origin của tab GỬI (không bao giờ tin theo payload của message), dùng lại đúng cơ chế phân giải origin -> connection sẵn có. Không đổi schema (`Transition.source: "auto-captured"` đã có sẵn từ trước).
- Ship cùng với code-import (bên dưới) như cách thứ hai trong hai cách không-thủ-công để lấp đầy `flows.json`/`screens.json` mà Decision Log 2026-07-23 từng hoãn lại sau v1; Track C (in-browser graph editor) vẫn đang trong kế hoạch. Xem mục "Tự động ghi lại screen transition" trong `docs/run-guide.md` và mục "Các pattern giao diện Auto-capture (Track B)" trong `docs/design-system.md`.
...
- Decision Log ngày 2026-07-23 từng hoãn việc này vào "v1.5"; đây là Track A của lát cắt đó (Track B auto-capture và Track C sửa trong extension vẫn đang được lên kế hoạch).
...
- Dữ liệu soạn tay ở v1 (chưa có editor trong extension cho flows/screens); `flows.json`/`screens.json` cũng có thể được sinh từ source TypeScript qua CLI `@specpin/import-flows` (đã ship, xem trên) thay vì soạn tay. Demo app có sẵn ví dụ hoàn chỉnh tại `examples/demo-react-app/.specs/flows.json` và `screens.json`, kết hợp cả hai: entry soạn tay sống cạnh một flow/screen import-owned.
