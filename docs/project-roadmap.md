...
- An opt-in, **default-OFF** record mode (Options -> **Auto-capture**) observes SPA + full-page navigation and derives a privacy-scrubbed screen transition per navigation: a generalized `urlGlob` for each screen (query and hash dropped; id-like path segments generalized to `**` - the same generalization approach as the `react-router` import adapter) and the edge between them. Captured transitions are buffered as drafts in a local, per-project, bounded `storage.local` ring buffer (`background/capture-buffer.ts`) - never written to `.specs/` and never uploaded.
...
- Privacy/consent UX: an unmissable **Recording navigation** indicator (pulsing dot) on both the Options card and a graph-panel banner, each with a reachable **Turn off** control; an in-product privacy statement stating exactly what is/isn't captured and that nothing reaches `.specs/` before an explicit Approve; a **Clear all captured** action (per project) for the draft buffer; and empty/full buffer-state messaging in the graph panel banner.
- New privileged messages `CLEAR_CAPTURE_BUFFER`/`APPROVE_CAPTURED_TRANSITION`/`DISCARD_CAPTURED_TRANSITION` (extension-page only) plus unprivileged `RECORD_CAPTURED_TRANSITION`/`GET_CAPTURE_BUFFER`; the background resolves a capture's owning project from the SENDER tab's origin (never trusted from the message payload), reusing the existing origin -> connection resolution. No schema change (`Transition.source: "auto-captured"` already existed).
