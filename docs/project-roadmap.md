...
- Ships alongside code-import (below) as the second of the two non-manual ways to populate `flows.json`/`screens.json` the 2026-07-23 Decision Log deferred past v1; Track C (in-browser graph editor) remains planned. See `docs/run-guide.md` "Auto-capturing screen transitions" and `docs/design-system.md` "Auto-capture (Track B) UI patterns".
...
- Decision Log 2026-07-23 deferred this as part of "v1.5"; this ships Track A of that slice (Track B auto-capture and Track C in-extension editing remain planned).
...
- Manual-authored data in v1 (no in-extension editor for flows/screens yet); `flows.json`/`screens.json` can also be populated from TypeScript source via the `@specpin/import-flows` CLI (shipped below) instead of by hand. The demo app ships worked examples at `examples/demo-react-app/.specs/flows.json` and `screens.json` combining both: hand-authored entries alongside an import-owned flow/screen.
