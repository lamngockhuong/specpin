...
- Ship-readiness polish: a dirty-state guard (any unsaved draft mutation marks the panel dirty) with a confirm-discard-or-save prompt before leaving Edit mode, switching project, or switching dataset, plus the browser's own leave-page warning on tab close/reload; a single-step **Undo** (last mutation only, by design - not a full history stack); and an orphaned-shot **Save** confirm (specshot integration) naming how many `.specs/shots/*.shot.json` artifacts a removed screen would orphan, or a generic caution when the shot inventory can't be verified - never a hard block.
- See `docs/run-guide.md` "Editing flows/screens in the browser" and `docs/design-system.md` "In-browser graph editor (Track C) UI patterns".

**Runtime auto-capture (Track B) shipped (2026-07-25)** on branch `main`:
- An opt-in, **default-OFF**, **per-project** record mode observes SPA + full-page navigation and derives a privacy-scrubbed screen transition per navigation: a generalized `urlGlob` for each screen (query and hash dropped; id-like path segments generalized to `**` - the same generalization approach as the `react-router` import adapter) and the edge between them. Recording is enabled per project (`Connection.recordEnabled` / `ManualBatch.recordEnabled`), not device-wide: the content-script recorder attaches to a page only when some project serving it is record-enabled, and a stabilized project can be switched off individually. Captured transitions are buffered as drafts in a local, per-project, bounded `storage.local` ring buffer (`background/capture-buffer.ts`) - never written to `.specs/` and never uploaded.
- Graph panel review: the screens dataset overlays the draft buffer onto the committed graph as dashed, reduced-opacity "ghost" nodes/edges (`graph-ghost.ts`'s pure `overlayGhostBuffer`, rendered by the existing `graph-svg.ts` with new `.pending` style rules - no separate renderer). Clicking a ghost edge opens an inline **Approve / Discard** panel; Approve merges it into `screens.json` with `"source": "auto-captured"` through the SAME provenance-preserving merge helper (`graph/graph-write-back.ts`) code-import and manual authoring share, never clobbering an existing manual/imported entry with the same id; Discard drops the draft with no `.specs/` write.
- Privacy/consent UX: a per-project **Record** toggle on each Options project row (beside its enable/disable switch) and a per-project recording banner in the graph panel with a **Turn on**/**Turn off** control; an in-product privacy statement stating exactly what is/isn't captured and that nothing reaches `.specs/` before an explicit Approve; a **Clear all captured** action (per project) for the draft buffer; and off/empty/full buffer-state messaging in the graph panel banner.
- New privileged messages `SET_LOCAL_BATCH_RECORD_ENABLED`/`CLEAR_CAPTURE_BUFFER`/`APPROVE_CAPTURED_TRANSITION`/`DISCARD_CAPTURED_TRANSITION` (extension-page only) plus unprivileged `RECORD_CAPTURED_TRANSITION`/`GET_CAPTURE_BUFFER` and a lightweight `RECORD_TARGETS_CHANGED` broadcast; the background resolves the serving projects from the SENDER tab's origin (never trusted from the message payload) and **fans a capture out to every record-enabled project serving it** (origin is host-level, so there is no single owner). No schema change (`Transition.source: "auto-captured"` already existed; `recordEnabled` is extension-local state, not a `.specs/` field).
- Ships alongside code-import (below) and the in-browser graph editor (above) as the three non-manual/interactive ways to populate `flows.json`/`screens.json` the 2026-07-23 Decision Log deferred past v1 - all three now shipped. See `docs/run-guide.md` "Auto-capturing screen transitions" and `docs/design-system.md` "Auto-capture (Track B) UI patterns".

**Code-import (`@specpin/import-flows`) shipped (2026-07-24)** on branch `main`:
- New standalone package `@specpin/import-flows`, a per-repo **devDependency CLI** (`specpin-import-flows`) that populates `.specs/flows.json`/`screens.json` from a consumer repo's own TypeScript source, driven by a committed `.specs/import.config.json`. No dependency on the Go sidecar or the browser extension - like `prisma generate`, it is build-time tooling only; `typescript` is a peerDependency so it parses with the consumer repo's own compiler version.
- Two adapters read source via the TypeScript compiler API (parse-only, never executes consumer code): `fsm-table` extracts an exported transition-table const (array-of-edges or record form) into `FlowState[]`/`Transition[]`; `react-router` extracts route paths from JSX `<Route>` elements and/or a flat route-object array into `Screen[]` (`:param` segments generalized to `**`).
- Provenance-preserving merge: an id the config declares this run is import-owned (tracked in a committed `.specs/.import-owned.json` companion file) and gets wholesale refreshed every run; any other existing id is treated as hand-authored and is never touched, so manual and imported entries coexist in the same file. `screens.json`'s `transitions[]` is never written by import (stays owned by manual authoring). New `Transition.source: "imported"` value distinguishes generated edges from `"manual"`/`"auto-captured"`.
- `--dry-run` prints a diff without writing; `--check` exits non-zero if the output would be stale (a CI gate); the default run writes only the files that changed and refreshes `.import-owned.json`.
- Worked example: `examples/demo-react-app` ships a committed `.specs/import.config.json` importing a `deal-pipeline` flow from `src/data.ts`'s `DEAL_STAGE_TRANSITIONS` and a `reports` screen from `src/routes-catalog.ts`, alongside the pre-existing hand-authored `deal-status` flow and five hand-authored screens - demonstrating the two sources coexisting in one `.specs/` directory. See `docs/run-guide.md` "Importing flows/screens from code" and `docs/schema-reference.md`.
- Decision Log 2026-07-23 deferred this as part of "v1.5"; this ships Track A of that slice (Track B auto-capture and Track C in-extension editing have since shipped too - see above).
...
- Extension: a new full-page `graph` entrypoint (dagre layout + hand-drawn SVG, CSP-safe, no Shadow DOM needed since it's an extension-owned page) launched via an **Open graph view** button in the popup and side panel, carrying the launching tab's id so a later click can target it. Controls: graph<->table toggle, category filter tabs (flows group by `object`, screens group by `urlGlob` top segment), live search (highlights, does not hide), focus-on-click (dims non-adjacent nodes/edges), pan/zoom. Pure `config-to-graph.ts` derivation (unit-tested) feeds both the dagre adapter and the SVG/table renderers.
- Overlay highlight: clicking a `specId`-bearing node/edge sends a cross-tab message (graph tab -> background -> the tab the graph was launched from); the content script resolves the id against that page's live specs and reuses the existing highlight/scroll path (no new arrow-drawing overlay). When the spec isn't matched on that page, a "not on the current page - {page}" hint names the screen instead of failing silently.
- Manual-authored data in v1 (no in-extension editor for flows/screens yet - since shipped, see the in-browser graph editor entry above); `flows.json`/`screens.json` can also be populated from TypeScript source via the `@specpin/import-flows` CLI (shipped below) instead of by hand. The demo app ships worked examples at `examples/demo-react-app/.specs/flows.json` and `screens.json` combining both: hand-authored entries alongside an import-owned flow/screen.
...
- Draggable spec badge: when an "S" badge covers host UI, press-drag it (mousedown + move past a small threshold) to a temporary spot; a plain click still pins the tooltip, and the click that follows a real drag is swallowed. A dragged badge stops re-anchoring on scroll/resize (per-pin flag; other badges still dodge it via the collision set). The position is never persisted, resetting on reload/re-match. Tooltip renderer only (coverage markers untouched). Docs updated EN + VI + JA.
...
|------|----------|-----------|
| 2026-07-24 | Code-import ships as a standalone per-repo devDependency CLI (`@specpin/import-flows`), not a sidecar/extension feature | Extraction is build-time, static-AST work with no runtime state; a separate package keeps `typescript` (a peer dependency) out of the sidecar and extension bundles, and lets a repo adopt it (or not) independent of everything else Specpin ships |
| 2026-07-23 | Graph views render in a dedicated full-page extension tab, not the side panel | A ~200-node reference graph needs the width; the side panel's docked, narrow layout was rejected as too cramped. Mirrors the reference UX (LaughTale Docs Explorer), and the overlay highlight still runs on the *original* tab regardless of where the graph itself lives |
| 2026-07-23 | Layout engine: `@dagrejs/dagre`, own hand-rolled SVG renderer | Dagre is a modest, pure-JS layered-DAG layout - the right shape for flow/screen diagrams - vs. heavier `elkjs` (rejected as overkill for v1 scale) or a hand-rolled Sugiyama layout (rejected as wheel-reinvention). Rendering itself stays hand-rolled (CSP-safe SVG via existing DOM helpers) since the extension has no rendering dependency to reuse |
| 2026-07-23 | v1.5 (`specpin import-flows`, code-import) and v2 (auto-capture navigation) deferred past v1 | v1 ships the foundation only: two schemas, manual authoring, the panel, and the `specId` overlay link. Code-import is cheap but still net-new parsing work; auto-capture carries real privacy risk (URL/path PII, background recording) and needs the renderer proven first. Foundation-first sequencing (Path A) ships a usable, reviewable slice sooner |
