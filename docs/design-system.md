...
- **Connected-arrow animation** (`.graph-edge.arrow-active`): selecting or focusing a node - clicking it in view mode, or selecting it while Edit mode is on - animates every edge touching that node with a flowing dashed stroke toward its target, so the active node's connections read at a glance. Unlike `.selected`, this state isn't gated to Edit mode. Respects `prefers-reduced-motion`: the dash animation is dropped but the accent tint stays, so the state remains visible without motion.
