...
1. Kiểm tra `.specs/manifest.json` trong dự án và thêm domain của trang hiện tại (ví dụ: `localhost:3000`, `example.com`) vào mảng `domains`.
2. Hoặc bật **Áp dụng cho mọi trang** cho dự án đó trong trang Options (dùng thận trọng: đặc tả sẽ xuất hiện trên mọi trang bạn truy cập).
...
1. **Specpin đang tắt.** Popup sẽ hiển thị "Specpin đang tắt." Bật nó với công tắc trong popup hoặc nhấn `Alt+Shift+S`.
