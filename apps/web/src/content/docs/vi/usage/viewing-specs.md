...
- **Bỏ qua** (xuất hiện khi phần tử có neo ổn định - test-id, non-generated id, hoặc CSS selector duy nhất): Loại bỏ dấu như một quyết định cá nhân. Việc bỏ qua được lưu trong `storage.sync` theo origin, nên gap vẫn bị ẩn sau lần reload và trên các máy khác. Khi bạn bỏ qua một gap, tóm tắt coverage và số đếm **Capture all gaps (K)** cập nhật ngay lập tức trong popup hoặc side panel đang mở.
