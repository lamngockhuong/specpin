...
Trong trang Options, mục **Spec** có tab **Thủ công** nơi bạn có thể nhập spec mà không cần chạy `specpin serve`. Tải spec từ tệp (chọn `manifest.json` + một hoặc nhiều `*.spec.json`) hoặc dán bundle JSON.
