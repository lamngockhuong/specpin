...
Trong trang Options, mục **Spec** có tab **Thủ công** nơi bạn có thể nhập spec mà không cần chạy `specpin serve`. Tải spec từ tệp (chọn `manifest.json` + một hoặc nhiều `*.spec.json`, hoặc một tệp `<project>.specs.zip` đã export) hoặc dán bundle JSON. Chọn tệp zip đã export sẽ nhập lại nó trực tiếp; chỉ hỗ trợ tệp zip không nén do tính năng Export của Specpin tạo ra.
