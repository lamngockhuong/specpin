...
- **ブラウザ拡張機能**：堅牢なフィンガープリント（test-id、aria、selector、xpath、テキスト、位置）でspecをDOM要素にマッチさせます。
...
- **ローカルプロジェクト**：`browser.storage.local`に保存されたspec。拡張機能内で直接作成します（個人用、ポータブル、`.specs.zip`としてエクスポート可能）。

## 次のステップ

- [拡張機能をインストール](/ja/guide/install/)
