...
const themeVal = (path, mode) =>
  path.startsWith("theme.") ? tokens.themes[mode][path.slice(6)] : path.split(".").reduce((o, k) => o[k], tokens);
...
// All token paths (theme-scoped use the "theme." prefix).
function allPaths() {
  const p = [];
  for (const k of Object.keys(tokens.brand)) p.push(`brand.${k}`);
  for (const k of Object.keys(tokens.font)) p.push(`font.${k}`);
  for (const k of Object.keys(tokens.radius)) p.push(`radius.${k}`);
  for (const k of Object.keys(tokens.themes.light)) p.push(`theme.${k}`);
  return p;
}
...
const NAME_MAP = {
  accent: "brand.base", "accent-indigo": "brand.base", "accent-teal": "brand.base",
  "accent-hover": "brand.hover",
  "accent-on": "brand.on", "accent-text": "brand.on", "on-accent": "brand.on",
  "accent-glow": "theme.accentGlow", "accent-indigo-glow": "theme.accentGlow", "accent-teal-glow": "theme.accentGlow",
  "grad-top": "theme.gradTop", "grad-bottom": "theme.gradBottom",
  "font-ui": "font.ui", "font-mono": "font.mono",
  "radius-card": "radius.card", "card-radius": "radius.card",
  "radius-control": "radius.control", "control-radius": "radius.control",
  "bg-page": "theme.bg", "page-bg": "theme.bg",
  "bg-surface": "theme.surface", surface: "theme.surface",
  "bg-elevated": "theme.elevated", elevated: "theme.elevated",
  "elevated-bg": "theme.elevated", "card-bg": "theme.elevated", "input-bg": "theme.elevated",
  "bg-control": "theme.control",
  border: "theme.border", "border-hairline": "theme.border",
  "text-primary": "theme.text", "text-secondary": "theme.text2", "text-muted": "theme.text3",
  "status-green": "theme.successDot",
  "success-bg": "theme.successBg", "success-border": "theme.successBorder", "success-text": "theme.successText",
  "error-bg": "theme.errorBg", "error-border": "theme.errorBorder", "error-text": "theme.errorText",
  amber: "theme.warning", "amber-border": "theme.warningBorder", "amber-bg": "theme.warningBg",
};
...
if (process.argv.includes("--rebind")) {
  ...
  for (const file of penFiles) {
    ...
    bindings[file] = map;
    console.log(`${file}: bound ${Object.keys(map).length}/${Object.keys(doc.variables ?? {}).length}`);
  ...
  for (const file of penFiles) {
    ...
    if (!map) { console.warn(`  [warn] ${file}: no bindings, skipped`); continue; }
    const doc = JSON.parse(readFileSync(new URL(file, dir)));
    let changed = 0;
