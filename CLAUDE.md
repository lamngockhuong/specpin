...
Phase 1 MVP + 1.1 slices delivered: `specpin validate` + CI spec-lint, Manual source (multi-batch: append on import with duplicate warning, remove per batch, clear all, grouped by site), modal renderer, multi-language specs (object-only `LocalizedString` + in-extension language toggle/authoring), multi-project connections (SidecarRegistry, origin-gated aggregation, per-connection token isolation), and a side panel surface (Chrome `side_panel` / Firefox `sidebar_action`, inline spec detail, auto-refresh, `defaultSurface` toolbar-click preference). Still deferred: FileSystem Access source, hybrid fingerprint scoring, overlay + inline-badge renderers, Safari packaging, and `specpin generate` (AI).
